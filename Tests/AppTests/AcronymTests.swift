...
final class AcronymTests: XCTestCase {
    ...
    func testAcronymCanBeSavedWithAPI() throws {
        ...
        try app.test(.POST, acronymsURI, beforeRequest: { request in
            ...
            try app.test(.GET, acronymsURI, loggedInUser: user, afterResponse: { allAcronymsResponse in
    ...
    func testDeletingAnAcronym() throws {
        ...
        try app.test(.DELETE, "\(acronymsURI)\(acronym.id!)", loggedInRequest: true, afterResponse:  { response in
